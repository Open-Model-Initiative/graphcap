...
export function PerspectiveDebug({ data, schema }: PerspectiveDebugProps) {
	...
	useEffect(() => {
		...
		if (!data.provider) {
			console.error(
				`MISSING DATA: Provider information absent for perspective ${schema.name}`,
			);
		}
		if (!data.model) {
			console.error(
				`MISSING DATA: Model information absent for perspective ${schema.name}`,
			);
		}
		if (!data.options) {
			console.error(
				`MISSING DATA: Generation options absent for perspective ${schema.name}`,
			);
		} else if (Object.keys(data.options).length === 0) {
			console.warn(
				`NOTE: Options object is present but empty for perspective ${schema.name}`,
			);
		}
	}, [data, schema.name]);
	...
	return (
		<Box position="relative">
			...
			<Stack gap={3} direction="column" maxHeight="350px" overflow="auto" p={2}>
