...
def resize_image(
    ...
    logger.info(f"Resizing image: original dimensions {orig_width}x{orig_height}, target {target_width}x{target_height}")
    ...
    if not upscale and orig_width <= target_width and orig_height <= target_height:
        logger.info(f"Skipping resize: image {orig_width}x{orig_height} already smaller than target {target_width}x{target_height}")
        return img
