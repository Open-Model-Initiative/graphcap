...
const DATASETS_PATH = import.meta.env.VITE_DATASETS_PATH ?? '/workspace/datasets';
...
export type AddImageToDatasetResponse = z.infer<typeof AddImageToDatasetResponseSchema>;
...
export function useAddImageToDataset() {
  ...
  return useMutation({
    mutationFn: async ({ imagePath, datasetName }: { imagePath: string; datasetName: string }) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          } else if (response.status === 404 && errorText.includes('Image not found')) {
...
export function useDeleteDataset() {
  ...
  return useMutation({
    mutationFn: async (name: string) => {
      ...
      try {
        const response = await fetch(`${MEDIA_SERVER_URL}/api/datasets/${name}`, {
          method: 'DELETE',
        });
