 ...
 */
const InferenceProviderContext = createContext<InferenceProviderContextType>(defaultContextValue);
...
export function useInferenceProviderContext() {
  ...
  if (!context) {
    throw new Error('useInferenceProviderContext must be used within an InferenceProviderProvider');
  }
  return context;
}
...
export function InferenceProviderProvider({
  ...
  // Use the form hook
  const { control, handleSubmit, errors, providerName, onSubmit: onSubmitForm, watch, reset } = useProviderForm(initialData);
