...
type NumericOptionKey = 'temperature' | 'max_tokens' | 'top_p' | 'repetition_penalty';

// Configuration for option min/max values and steps
const OPTION_CONFIGS: Record<NumericOptionKey, { min: number; max: number; step: number; precision: number }> = {
...
export function GenerationOptionForm({
  ...
  const debouncedUpdateOptions = useCallback((newOptions: CaptionOptions) => {
  ...
  const handleSliderChange = (name: NumericOptionKey) => (details: { value: number[] }) => {
  ...
  const handleInputChange = (name: NumericOptionKey) => (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const getSliderValue = (key: NumericOptionKey): number[] => {
    const value = typeof localOptions[key] === 'number' ? localOptions[key] : DEFAULT_OPTIONS[key];
