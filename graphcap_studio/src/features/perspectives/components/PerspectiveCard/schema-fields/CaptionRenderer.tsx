...
import { CaptionProvider, useCaptionContext, CaptionData } from './CaptionContext';
...
export function CaptionRenderer({ schema, data, className = '' }: CaptionRendererProps) {
  return (
...
function CaptionContent({ className = '' }: CaptionContentProps) {
  ...
  const fieldsToRender = useMemo(() => {
    ...
    return Object.keys(data).map(key => ({
      ...
      is_complex: typeof data[key] === 'object' && data[key] !== null && !Array.isArray(data[key])
    })) as SchemaField[];
  }, [schema, data]);
