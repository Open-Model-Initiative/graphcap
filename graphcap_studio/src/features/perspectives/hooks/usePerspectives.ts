...
export class PerspectiveError extends Error {
  ...
  constructor(message: string, options: { code: string; statusCode?: number; context?: Record<string, any> }) {
    super(message);
  ...
  const query = useQuery<Perspective[], Error>({
    ...
    queryFn: async () => {
      try {
        ...
        console.debug(`Successfully fetched ${data.perspectives.length} perspectives`);
        ...
        throw new PerspectiveError(
          error instanceof Error ? error.message : 'Unknown error fetching perspectives',
          { 
    ...
    retry: (failureCount, error) => {
      ...
      if (error instanceof PerspectiveError) {
        if (['SERVER_CONNECTION_ERROR', 'MISSING_SERVER_URL'].includes(error.code)) {
          return false;
        }
      }
