...
export function ImageCarouselProvider({
  ...
  const thumbnailsRef = useThumbnailScroll({
    selectedIndex: initialSelectedImage ? Math.max(0, currentIndex - visibleStartIndex) : 0,
  ...
  useImagePreloader({
    ...
    enabled: normalizedPreloadOptions.enabled && !isLoading && !isEmpty && images.length > 0,
