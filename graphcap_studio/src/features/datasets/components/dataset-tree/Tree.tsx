...
function TreeContent({ className = "" }: TreeContentProps) {
	...
	return (
		<div ref={treeRef} className={`py-2 space-y-0.5 ${className}`} role="tree">
	...
	return (
		<div className="group transition-all duration-200">
			<TreeNode
				...
				icon={
					<TreeIcon type={item.iconType ?? (hasChildren ? "folder" : "file")} />
				}
				onClick={handleItemClick}
				onToggleExpand={handleToggleExpand}
				wrapperComponent={
					getWrapperComponent ? getWrapperComponent(item) : undefined
				}
				item={item}
				contextMenuActions={contextMenuActions}
			/>
