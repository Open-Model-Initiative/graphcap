...
import { useActionPanel, useLayoutHeight } from "./hooks";
...
export function MainLayout({
	...
	const leftPadding = leftPanel?.isExpanded
		? leftPanel.width
		: (leftPanel?.width ?? 0);
	const rightPadding = rightPanel?.isExpanded
		? rightPanel.width
		: (rightPanel?.width ?? 0);
		...
		<div ref={containerRef} className={styles.container}>
			...
			<main
				...
				style={{
					...
					paddingLeft:
						panelsReady && leftPanel ? `${leftPadding}px` : undefined,
					paddingRight:
						panelsReady && rightPanel ? `${rightPadding}px` : undefined,
