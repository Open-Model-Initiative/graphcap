...
const attemptUserInteractionCopy = (
  ...
  if (debug) console.log('[useClipboard] Attempting final fallback with user interaction');
  ...
  button.onclick = async () => {
    try {
      ...
      onError(finalErr instanceof Error ? finalErr : new Error('Final clipboard fallback failed'));
    }
...
export function useClipboard(options: UseClipboardOptions = {}): UseClipboardResult {
   ...
   */
  const handleCopyError = useCallback((err: unknown) => {
    debugLog('Copy failed:', err, true, debug);
    
    if (debug) {
      debugLog('Navigator:', typeof navigator, false, debug);
      debugLog('Clipboard:', typeof navigator !== 'undefined' ? typeof navigator.clipboard : 'N/A', false, debug);
    }
   ...
   */
  const copyWithClipboardAPI = useCallback(async (text: string): Promise<boolean> => {
  ...
  const copyToClipboard = useCallback(async (text: string) => {
    ...
    debugLog('Clipboard API available:', isClipboardAPIAvailable(), false, debug);
    ...
    try {
      ...
      if (!clipboardApiSuccess) {
        debugLog('Falling back to selection-based copy', undefined, false, debug);
