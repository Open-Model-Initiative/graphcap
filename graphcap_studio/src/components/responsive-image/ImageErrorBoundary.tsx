...
function ImageErrorFallback({
	...
	return (
		<div
			className={`flex flex-col items-center justify-center p-2 bg-gray-800/20 h-full w-full ${className}`}
		>
			{!compact && (
				<svg
			...
			<div
				className={`${compact ? "text-xs" : "text-sm"} text-red-400 text-center mb-1`}
			>
...
function createFallbackRender(className: string, compact: boolean) {
	return (props: FallbackProps) => (
		<ImageErrorFallback {...props} className={className} compact={compact} />
	);
}
...
export function ImageErrorBoundary({
	...
	return (
		<ErrorBoundary fallbackRender={createFallbackRender(className, compact)}>
			{children}
		</ErrorBoundary>
	);
}
